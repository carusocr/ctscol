...
my $ldcif  = 'c:/fisher_v0.8/Perl/.ldci.config';

our $proc_dir  = $fshyml->{proc}->{proc_dir};
our $dberr_dir = $fshyml->{dberr}->{dberr_dir};

our %telco_mysql = ();
load_ldci($ldcif,\%telco_mysql,':dbm');

sub load_ldci {

    my ($cfgf,$ref, $dlbl) = @_;
    my $cfgr = LoadFile($cfgf);

    $ref->{driver}   = $cfgr->{ $dlbl }->{ ':driver'   };
    $ref->{database} = $cfgr->{ $dlbl }->{ ':database' };
    $ref->{userid}   = $cfgr->{ $dlbl }->{ ':userid'   };
    $ref->{passwd}   = $cfgr->{ $dlbl }->{ ':passwd'   };
    $ref->{host}     = $cfgr->{ $dlbl }->{ ':host'     };
    $ref->{dbistr}   = sprintf("DBI:%s:host=%s;database=%s", 
			       $ref->{driver},
			       $ref->{host},
			       $ref->{database});
}
