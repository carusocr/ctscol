  ...
  while(getpin() eq TRUE and _ntries < 3)
	...
	_cr_pin = procread("pin",_cr_pid);	
        _cr_numval = _cr_number;
      	...
      	switch(cfibpgen(_cr_line,_cr_pid))
      	case 2:
      	     _ce_pin = procread("ce_pin",_cr_pid);
      	case 1:   
	...
	case "calling":
	  ...
	  _cfib_tod = CFIB_PROMPTS & procread("topic_file",_cr_pid);	
	  ...
	  switch(1)
	  case procread("recbytes",_cr_pid) > FULLREC_LEN:
	  	procwrit("termstat","FULLREC");
	  case procread("recbytes",_cr_pid) > SHORTREC_LEN:
	  	procwrit("termstat","SHORTREC");
	  case procread("recbytes",_cr_pid) > MINREC_LEN:
	  ...
	  sc_play(_cr_line,CFIB_SURVEYQA,PBMODE);	
  	  cfibsurv(_call_id, "cplang", dtmfresp(_cr_line,CFIB_SURVEYRA,1));

  	  sc_play(_cr_line,CFIB_SURVEYQB,PBMODE);	
  	  cfibsurv(_call_id, "cpgend", dtmfresp(_cr_line,CFIB_SURVEYRB,1));

	  sc_play(_cr_line, CFIB_PINQRY, PBMODE);
	  digiplay(line, _ce_pin);
	  procwrit("cepin_confirm", dtmfresp(_cr_line, CFIB_PINCONFIRM,6));

  	  sc_play(_cr_line,CFIB_THANKYOU,PBMODE);
  	  if(dtmfresp(_cr_line,CFIB_COMMENT,1) streq "1")
