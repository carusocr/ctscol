...
my $sth = $dbh->prepare( "select subj_id, active, calls_done, max_allowed, group_id,
                           IFNULL(subgroup_id,'0001_0000') from rats_subj where pin=\'$pin\'");

$sth->execute;
my ($subj_id,$active,$calls_done,$max_allowed,$group_id,$subgroup_id) = $sth->fetchrow;
$sth->finish;

$sth = $dbh->prepare("select subj_id from rats_subj where subgroup_id = \'$subgroup_id\'
                      and pin <> \'$pin\'");
$sth->execute;
...
if ( !defined( $subj_id )) {
    ...
    procwrite($proc_id,"rec_length",600);
    ...
    $sth = $dbh->prepare("select pin from rats_subj where subj_id=?");
