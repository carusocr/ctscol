...
my $sth = $dbh->prepare( "insert into rats_br_calls ( cra_side_id, crb_side_id, call_date )
...
$sth = $dbh->prepare( "update rats_io_calls set subj_id=?, bridged_to=?, br_call_id=? where side_id=?" );
