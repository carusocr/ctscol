...
$s = $dbh->prepare("select ms.subj_id,IFNULL(ts.gender,'X'),IFNULL(ts.state,'ZZ') from mx7spa_subj ms, telco_subjects ts
...
#$s = $dbh->prepare("update mx7spa_topics set tod_yn = 'N'");
#$s->execute;
#$s->finish;

#$s = $dbh->prepare("update mx7spa_topics set tod_yn = 'Y' where topic_id = '$newtod'");
#$s->execute;
#$s->finish;
