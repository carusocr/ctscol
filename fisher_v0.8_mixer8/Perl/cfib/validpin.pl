...
my $sth = $dbh->prepare( "SELECT p.id AS participant_id, p.active
                          FROM participants p
                          JOIN $lui{database}.enrollments e ON p.enrollment_id = e.id
                          JOIN $lui{database}.collections c ON e.collection_id = c.id
                          WHERE p.pin = $pin AND c.name = \'$collection{name}\'");
...
my ($subj_id,$active) = $sth->fetchrow;
$sth->finish;
...
# }
    ...
    my $sth = $dbh->prepare("UPDATE participants
                             SET conversation_in_progress = 1, updated_at = NOW()
    ...
    procwrite($proc_id,"rec_length",300);
